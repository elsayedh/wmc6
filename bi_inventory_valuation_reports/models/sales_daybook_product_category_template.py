...
class sales_daybook_product_category_report(models.AbstractModel):
    ...
    def _get_lines(self, data):
        ...
        for product in  product_res:
            ...
            # current_price = fields.Float("CP", compute="_get_price")
            #
            # @api.depends('value', 'create_date')
            # def _get_price(self):
            #     for pro in self:
            #         sum_val = sum(self.env['stock.valuation.layer'].search(
            #             [('product_id', '=', pro.product_id.id), ('create_date', '<=', pro.create_date)]).mapped(
            #             'value'))
            #         sum_qty = sum(self.env['stock.valuation.layer'].search(
            #             [('product_id', '=', pro.product_id.id), ('create_date', '<=', pro.create_date)]).mapped(
            #             'quantity'))
            #         pro.current_price = sum_val / sum_qty
